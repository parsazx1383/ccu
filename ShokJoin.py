        ...
        if chat_id != Admin:
            try:
                ...
                if not is_member:
                    ...
                    # ارسال پیام عضویت و توقف اجرای تابع اصلی
                ...
                if chat_id == Admin:
                    ...
                    return  # کاربران عادی اگر خطا در بررسی عضویت داشته باشند، نتوانند ادامه دهند
...
لطفا عکس از کارتی که میخواهید با آن خرید انجام دهید ارسال کنید...**""",
    ...
    # بررسی مجدد عضویت
        is_member = await check_membership(call.from_user.id)
        
        if is_member:
            # عضویت تأیید شد - برو به منوی اصلی
            await app.delete_messages(call.message.chat.id, call.message.id)
            
            # دریافت اطلاعات کاربر
            user_data = get_data("SELECT expir FROM user WHERE id = %s", params=[call.from_user.id])
            expir = user_data.get("expir", 0) if user_data else 0
            ...
            await app.send_message(call.from_user.id, start_message, reply_markup=keyboard)
            ...
            await app.answer_callback_query(
                call.id, 
                text="❌ هنوز در کانال عضو نشده‌اید! لطفا ابتدا در کانال عضو شوید و سپس بر روی دکمه کلیک کنید.", 
                show_alert=True
            )
